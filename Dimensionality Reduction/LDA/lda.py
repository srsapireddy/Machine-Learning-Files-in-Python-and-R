# Linear Discriminant Analysis -> Feature extraction technique
# Here we are extracting some new independent variables that will seperate the most of the classes of the dependent variable. This makes LDA a supervied dimensionality reduction technique as we are using the dependent variable in this technique.
...
X_train = lda.fit_transform(X_train, y_train) # This will extract all the linear discriminant components of the training set.
X_test = lda.transform(X_test)
# n_components --> That is two new independent variables that are going to seperated the classes of the dependent variable. Here we are goinf to look for two linear discriminant variables.
